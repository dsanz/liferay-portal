 ...
 */
@Component(
	property = "react.props.provider.order:Integer=200",
	service = ReactPropsProvider.class
)
public class ObjectEntryReactPropsProvider implements ReactPropsProvider {
	...
	public Map<String, Object> getObjectEntryReactProps(
		...
		ObjectEntry dataSetObjectEntry = _getDataSetObjectEntry(
			httpServletRequest, fdsName);

		ObjectDefinition dataSetObjectDefinition =
			_getDataSetEntryObjectDefinition(httpServletRequest);
		...
		try {
			...
			_log.error(
				"Unable to render data set from object entries", exception);
	...
	public boolean isAvailable(
		...
		if (_getDataSetObjectEntry(httpServletRequest, fdsName) != null) {
			return true;
		}
	...
	private JSONObject _getCreationMenuJSONObject(
		...
		JSONArray jsonArray = JSONUtil.toJSONArray(
			...
			(ObjectEntry objectEntry) -> {
					...
					for (DropdownItem dropdownItem : dropdownItems) {
						...
						creationMenuItemsJSONArray.put(
							JSONUtil.put(
								...
								"href",
								_getValueIfNotNull(dropdownItem.get("href"))
							).put(
								"icon",
								_getValueIfNotNull(dropdownItem.get("icon"))
							).put(
								"label",
								_getValueIfNotNull(dropdownItem.get("label"))
							).put(
								"target",
				...
				return JSONUtil.put(
					...
					JSONUtil.put(
						"disableHeader",
						Validator.isNull(properties.get("title"))
					).put(
		...
		return JSONUtil.put("primaryItems", resultJSONArray);
	}

	private ObjectDefinition _getDataSetEntryObjectDefinition(
		HttpServletRequest httpServletRequest) {
	...
	private ObjectEntry _getDataSetObjectEntry(
		HttpServletRequest httpServletRequest, String fdsName) {

		ObjectEntry dataSetObjectEntry = null;

		if (Validator.isNotNull(fdsName)) {
			try {
				dataSetObjectEntry = _getObjectEntry(
					_portal.getCompanyId(httpServletRequest), fdsName,
					_getDataSetEntryObjectDefinition(httpServletRequest));
			}
			catch (Exception exception) {
	...
	private static final Log _log = LogFactoryUtil.getLog(
		ObjectEntryReactPropsProvider.class);
